...
namespace sep.backend.v1.Validators
{
    public class TeacherValidator : AbstractValidator<CreateTeacherDTO>
    {
        public TeacherValidator()
        {
            ...
            RuleFor(u => u.DateOfBirth)
                ...
                .Must(BeAtLeast20YearsOld).WithMessage(Messages.DATE_OF_BIRTH_YEAR_ENOUGH);
            ...
            RuleFor(u => u.SubjectIds)
                .Cascade(CascadeMode.Stop)
                .NotEmpty().WithMessage(Messages.SUBJECT_ASSIGN);
        }

        private bool BeAtLeast20YearsOld(DateTime? dateOfBirth)
        {
    ...
    public class TeacherUpdateValidator : AbstractValidator<UpdateTeacherDTO>
    {
        ...
        public TeacherUpdateValidator(ApplicationContext context)
        {
            _context = context;
            RuleFor(u => u.FirstName)
               .Cascade(CascadeMode.Stop)
    ...
    public class TeacherUpdateProfileValidator : AbstractValidator<UpdateProfileTeacherDTO>
    {
        private readonly ApplicationContext _context;
        ...
        public TeacherUpdateProfileValidator(ApplicationContext context)
        {
            _context = context;
            ...
            RuleFor(u => u.FirstName)
               .Cascade(CascadeMode.Stop)
               .NotEmpty().WithMessage(Responses.FirstNameRequired)
               .Length(2, 15).WithMessage(Messages.NAME_LENGTH)
               .Matches(@"^[\p{L}]+$").WithMessage(Messages.FIRST_NAME_INVALID);
            RuleFor(u => u.LastName)
                .Cascade(CascadeMode.Stop)
                .NotEmpty().WithMessage(Responses.LastNameRequired)
                .Length(2, 25).WithMessage(Messages.NAME_LENGTH)
                .Matches(@"^[\p{L}\s]+$").WithMessage(Messages.LAST_NAME_INVALID);
            RuleFor(u => u.Email)
                .Cascade(CascadeMode.Stop)
        ...
        private bool BeUniqueEmail(string email, int teacherId)
        {
            if (!_context.Teachers.Any(u => u.Id == teacherId))
            {
                throw new NotFoundException();
            }
            return !_context.Teachers.Any(u => u.Email == email && u.Id != teacherId);
        }
        private bool BeAtLeast20YearsOld(DateTime? dateOfBirth)
        {
            if (dateOfBirth == null) return false;
