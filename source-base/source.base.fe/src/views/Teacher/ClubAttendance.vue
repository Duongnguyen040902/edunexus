<template>
  <div class="container-fluid flex-grow-1 container-p-y">
          ...
          <div class="card-body">
            ...
            <div class="d-flex justify-content-between align-items-end">
              <div class="role-heading">
                ...
                <a href="javascript:;" @click="handleOpenModal(true, attendance.date)" class="role-edit-modal"
                ><span>Chỉnh sửa</span></a
                >
              </div>
              <a @click="handleViewDetail(attendance.date)"><i class="bx bx-copy bx-md text-muted"></i></a>
            </div>
          </div>
        </div>
      </div>
    ...
    <CreateAndUpdateAttendanceModal
      ...
      v-bind:attendance-record="attendanceRecords"
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleOpenModal = async (isUpdateMode: boolean, date: Date) => {
      if (isUpdateMode) {
        await fetchAttendanceRecord({
          entityId: clubId.value,
          session: AttendanceType.Club,
          type: AttendanceSection.Extracurricular,
    ...
    const handleViewDetail = async (date: Date) => {
      await fetchAttendanceRecord({
        entityId: clubId.value,
        session: AttendanceType.Club,
        type: AttendanceSection.Extracurricular,
